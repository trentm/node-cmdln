...
function CmdlnError(options) {
    ...
    var extra = Object.keys(options).filter(
        function onKey(k) { return ['cause', 'message'].indexOf(k) === -1; });
...
UnknownCommandError.prototype.cmdlnErrHelpFromErr =
    ...
    var ge50 = [];
    candidates && candidates.forEach(function onCandidate(candidate) {
        if (candidate[0] >= 0.3) {
...
function Cmdln(config) {
    ...
    prototypes.forEach(function onProto(proto) {
        Object.keys(proto)
            .filter(function onProp(funcname) { return /^do_/.test(funcname); })
...
Cmdln.prototype.bashCompletionSpec = function bashCompletionSpec(opts) {
    ...
    var aliases = [];
    var allAliases = [];
    Object.keys(this._nameFromAlias).sort().forEach(function onAlias(alias) {
        if (alias === '?') {
...
Cmdln.prototype.helpFromSubcmd = function helpFromSubcmd(alias) {
    ...
    if (handler.help) {
        ...
        return function subCliHelp(_subcmd, opts, args, cb) {
            handler.do_help('help', opts, args.slice(1),
                function onHelp(helpErr) {
                    cb(helpErr || false);
                }
            );
        };
...
Cmdln.prototype.dispatch = function dispatch(dispatchOpts, callback) {
    ...
    if (!handler) {
        this.defaultHandler(subcmd, opts, args, finish);
        return;
    }
...
Cmdln.prototype.defaultHandler = function defaultHandler(
    ...
    cb(new UnknownCommandError(subcmd));
};
    ...
    cli.main(options.argv, function mainFinish(err) {
        ...
        if (err && showErr) {
            ...
            if (code === 'NoCommand' && !options.showNoCommandErr) {
                ...
                var errDetails;
                // `.ase_errors` is an verror impl detail.
                if (err.name === 'MultiError' && err.ase_errors) {
