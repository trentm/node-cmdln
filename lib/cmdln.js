...
Cmdln.prototype.bashCompletionSpec = function bashCompletionSpec(opts) {
    ...
    var hFromSubcmd = {};
    self._subcmdOrder.forEach(function (name) {
        ...
        // TODO: Excluding hidden ones: Make this optional. Hide by default.
        // if (handler.hidden)
        //     return;

        hFromSubcmd[name] = handler;
        (handler.aliases || []).forEach(function (alias) {
            if (alias === '?') {
                // '?' as a Bash completion is painful. Also, '?' as a default
                // alias for 'help' should die.
                return;
            }
            hFromSubcmd[alias] = handler;
        });
    });
    spec.push(format('local cmd%s_subcmds="%s"', context,
        Object.keys(hFromSubcmd).sort().join(' ')));

    // Subcmds.
    Object.keys(hFromSubcmd).sort().forEach(function (name) {
        spec.push('');
        var context_ = context + '__' + name.replace(/-/g, '_');
        var handler = hFromSubcmd[name];
...
Cmdln.prototype.dispatch = function dispatch(subcmd, argv, callback) {
    ...
    if (typeof (handler.main) === 'function') {
        ...
        (function callCmdlnHandler(subcmd, opts, args, cb) {
            ...
            handler.main(argv, function (err, subsubcmd) {
                if (subsubcmd) {
                    subsubcmd = subcmd + ' ' + subsubcmd;
                }
                cb(err, subsubcmd);
            });
        }).call(this, subcmd, opts, args, callback);
    } else {
