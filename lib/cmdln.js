...
var COMPLETION_TEMPLATE = fs.readFileSync(
        path.join(__dirname, '../etc/completion.bash.templ'),
        'utf8');
...
Cmdln.prototype.bashCompletion = function bashCompletion(cb) {
    var self = this;

    var args = parseOptions(self.options);
    var commands = {};
    ...
    self._subcmdOrder.forEach(function (name, idx) {
        ...
        var opts = parseOptions(handler.options);
    ...
    // render template
    console.log(template(COMPLETION_TEMPLATE, data));
...
// replace {{variable}} in `s` with what's in `d`
