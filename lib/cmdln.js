...
function renderTemplate(s, d) {
    return s.replace(/{{([a-zA-Z]+)}}/g, function onVar(match, key) {
        return hasOwnProperty(d, key) ? d[key] : match;
    });
}
...
function main(cli, options) {
    ...
    var finale;
    if (hasOwnProperty(options, 'finale')) {
        assert.ok(
    ...
    if (hasOwnProperty(options, 'callback')) {
