 ...
 * - collect XXXs from below and from node-triton/lib/cli.js
 * - cmdln docs
 * - tests
 ...
 * - would like a clean up of var name usage:
 *      'subcmd': is that the handler func? or the string? or either?
 *      'cmd': e.g. with the templates, deprecate this?
 */
 ...
 *
 * XXX: move this synopsisFromOpt to dashdash
 */
OptionError.prototype.cmdlnErrHelpFromErr = function optionErrHelpFromErr(err) {
    ...
    var synopsisFromOpt = function (o) {
        ...
        var type = dashdash.getOptionType(o.type);
    ...
    if (options) {
        ...
        for (var i = 0; i < options.length; i++) {
            var synopsis = synopsisFromOpt(options[i]);
            if (!synopsis) {
                continue;
 ...
 *
 * - OptionError: Show the command's synopses, if available.
 ...
 * @param callback {Function} `function (finiErr)`.
 */
